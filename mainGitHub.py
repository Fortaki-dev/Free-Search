...
from PyQt6.QtCore import (
    ...
    QEasingCurve, QPoint, QRect, QEventLoop, QObject, QUrl
)
...
from PyQt6.QtWidgets import (
    ...
    QSlider, QCheckBox, QInputDialog, QFileDialog, QGroupBox, QApplication
)
...
import asyncio
import aiohttp
class SafeBrowsingChecker(QObject):
    _check_finished = pyqtSignal(str, bool, object, object)
    ...
    def __init__(self, api_key, parent=None):
        ...
        self._check_finished.connect(self._handle_callback)

    def check_url(self, url: str, callback):
        if not self.enabled:
            callback(True, None)
            return

        if url.startswith(("http://127.0.0.1:5000", "http://localhost", "about:blank", "file://")):
            callback(True, None)
            return
        ...
        if domain in self.cache:
            ...
            if now - timestamp < self.cache_ttl:
                callback(is_safe, threat_type)
                return
            else:
                del self.cache[domain]

        # Если в кэше нет — отправляем асинхронный сетевой запрос
        self._send_request_async(url, domain, callback)

    def _send_request_async(self, url, domain, callback):
        payload = {
        ...
        # Создаем таймер таймаута индивидуально для этого запроса
        ...
        def on_finished():
            if timeout_timer.isActive():
                timeout_timer.stop()
            self._process_reply(reply, url, domain, callback)

        def on_timeout():
            if reply.isRunning():
                print(f"[SafeBrowsing] Таймаут запроса для {url}")
                reply.abort() 

        reply.finished.connect(on_finished)
        timeout_timer.timeout.connect(on_timeout)
        timeout_timer.start(10000) # 10 секунд лимит

    @staticmethod
    def error(title, details):
        file_path = resource_path("Error.html")
        try:
            with open(file_path, "r", encoding="utf-8") as f:
                html_content = f.read()
            html_content = html_content.replace("{ERROR_TITLE}", title)
            html_content = html_content.replace("{ERROR_DETAILS}", details)
            return html_content
        except Exception as e:
            return f"<h1>Критическая ошибка браузера</h1><p>Не найден файл Error.html. Текст ошибки: {title} - {details}</p>"

    def _process_reply(self, reply, url, domain, callback):
        is_safe, threat_type = True, None
        
        # Если запрос был отменен по таймауту, reply.error() вернет OperationCanceledError
        if reply.error() != QNetworkReply.NetworkError.OperationCanceledError:
            # 1. Задаем текст технической ошибки
            title = "[SafeBrowsing] Ошибка сети"
            details = f"Не удалось связаться с серверами проверки безопасности. Код: {reply.errorString()}"
                
            # 2. Загружаем ваш красивый Error.html с этим текстом
            error_html = error(title, details)
                
            # 3. Отправляем в браузер для отображения через setHtml()
            self._check_finished.emit(url, True, None, (callback, error_html))

        reply.deleteLater()
        
        # Сохраняем в кэш
        self.cache[domain] = (is_safe, threat_type, time.time())
        
        # Безопасно отправляем результат в callback через Qt Сигнал
        self._check_finished.emit(url, is_safe, threat_type, callback)

    def _handle_callback(self, url, is_safe, threat_type, callback):
        # Вызываем переданную пользователем функцию
        callback(is_safe, threat_type)
    print("[INFO] Google Safe Browsing Включена")
...
def acceptNavigationRequest(self, url, navigation_type, is_main_frame):
    if is_main_frame:
        ...
        # 1. Объявляем функцию, которая выполнится ТОЛЬКО КОГДА прилетит ответ от Google
        def handle_check_result(is_safe, threat_type):
            if is_safe:
                # Сайт чист — временно отключаем фильтр (чтобы избежать бесконечного цикла) и загружаем URL
                self.browser_window.safe_browsing.enabled = False
                self.setUrl(url)
                self.browser_window.safe_browsing.enabled = True
            else:
                # Сайт ОПАСЕН — переводим технический термин на человеческий язык
                threat_names = {
                ...
                # Показываем ваше всплывающее окно с предупреждением
                ...
                if reply == QMessageBox.StandardButton.Yes:
                    # Пользователь — бесстрашный хакер, пускаем его на свой страх и риск
                    self.browser_window.safe_browsing.enabled = False
                    self.setUrl(url)
                    self.browser_window.safe_browsing.enabled = True
                else:
                    # Пользователь нажал "Нет" — загружаем ваш красивый Error.html
                    try:
                        from security import load_error_page
                        title = f"[SafeBrowsing] Обнаружен: {threat_name}"
                        details = f"Браузер FreeSearch заблокировал переход на {url_str}, так как этот ресурс находится в черных списках безопасности."
                        error_page = load_error_page(title, details)
                        self.setHtml(error_page)
                    except Exception as e:
                        print(f"Не удалось загрузить Error.html: {e}")
                        self.setHtml(f"<h1>Сайт заблокирован</h1><p>Угроза: {threat_name}</p>")

        # 2. ЗАПУСКАЕМ ПРОВЕРКУ в Google Safe Browsing
        self.browser_window.safe_browsing.check_url(url_str, handle_check_result)
        
        # 3. МГНОВЕННО ОТМЕНЯЕМ стандартную загрузку. 
        # Страница не начнет открываться, пока не отработает handle_check_result!
        return False

    # Если это не фрейм (например, подгрузка картинки или скрипта внутри сайта), разрешаем стандартно
...
from cryptography.hazmat.primitives.kdf.pbkdf2 import PBKDF2HMAC
from cryptography.hazmat.primitives import hashes
def password_gen(master_password: str, salt: bytes) -> bytes:
    kdf = PBKDF2HMAC(
...
def save_passwords(data):
    salt = os.urandom(16)
    key = password_gen(master_password, salt)
    cipher = Fernet(key)
    jsons = jsons.dump(data)
    bytes_data = jsons.encode('utf-8')
    encrypted_data = cipher.encrypt(bytes_data)

    with open(PASSWORD_FILE, 'wb') as f:
        f.write(salt + encrypted_data)
    print("[INFO] База паролей успешно зашифрована и сохранена")

def load_passwords():
    if not os.path.exists(PASSWORD_FILE):
        return {}

    with open(PASSWORD_FILE, 'rb') as e:
        file_contect = f.read()

    if len(file_contect) < 16:
        return {}

    salt = file_contect[:16]
    encrypted_data = file_contect[16:]
    key = password_gen(master_password, salt)
    cipher = Fernet(key)

    try:
        bytes_data = cipher.decrypt(encrypted_data)
        jsons = bytes_data.decode('utf-8')
        return json.loads(jsons)
    except Exception:
        print("[ERROR] Неверный мастер-пароль или файл поврежден!")
        return None 
...
class BrowserWindow(QMainWindow):
    ...
    def check_flask_status(self):
        """Фоновая проверка Flask без фриза интерфейса"""
        # Создаем внутренний счетчик попыток, если его еще нет
        if not hasattr(self, '_flask_attempts'):
            self._flask_attempts = 0
            
        try:
            # Пытаемся достучаться до Flask (таймаут очень маленький, чтобы не фризить)
            response = requests.get(f'http://127.0.0.1:{PORT}', timeout=0.1)
            if response.status_code == 200 or response.status_code == 404:
                print("[LOG] Flask готов и отвечает!")
                return 
        except Exception:
            pass

        self._flask_attempts += 1
        
        if self._flask_attempts < 30:
            # Если Flask еще не готов, ждем 100 мс и проверяем СНОВА, 
            # при этом интерфейс браузера остается полностью рабочим и отзывчивым!
            QTimer.singleShot(100, self.check_flask_status)
        else:
            # Если за 30 попыток (около 3 секунд) не завелся — показываем ошибку
            QMessageBox.critical(self, "Ошибка", "Не удалось запустить внутренний сервер.")
            self.close()

    def handle_load_finished(self, success):
        # Если страница НЕ загрузилась (ошибка сети, SSL или блокировка Chromium)
        if not success:
            script_dir = os.path.dirname(os.path.abspath(__file__))
            error_path = os.path.join(script_dir, "Error.html")
            ...
            if os.path.exists(error_path):
                ...
                fail_url = self.browser.url().toString()
                danger_message = f"Браузер FreeSearch заблокировал небезопасный переход на: {fail_url}."
                
                # Твои маркеры из HTML:
                ready_html = html_content.replace("{ERROR_TITLE}", "⚠️ Ошибка безопасности сети")
                ready_html = ready_html.replace("{ERROR_DETAILS}", "Запрос заблокирован внутренней системой цифровой гигиены. Возможно, у сайта поддельный сертификат или протокол не поддерживается.")
                ready_html = ready_html.replace("{ERROR_TEXT}", danger_message)

                # Рендерим ТВОЙ HTML поверх стандартного хромиумовского
                base_url = QUrl.fromLocalFile(script_dir + "/")
                self.browser.setHtml(ready_html, base_url)


    import re
    from urllib.parse import urlparse
    def safe_download_site(self, user_url): # Убрали browser_view, так как есть self.browser
        try:
            import re
            from urllib.parse import urlparse
            ...
            # 🔥 ВЕРНУЛИ ПРОВЕРКУ: блокируем ТОЛЬКО если протокол НЕ http и НЕ https
            if parsed.scheme not in ['http', 'https']:
                ...
                if os.path.exists(error_path):
                    ...
                    title_text = "⚠️ Заблокирована попытка обхода защиты"
                    details_text = "Браузер FreeSearch предотвратил небезопасное действие. Внешний процесс скачивания заблокирован во избежание выполнения вредоносного кода на вашем компьютере."
                    danger_message = f"Причина: Протокол '{parsed.scheme}' запрещен правилами цифровой гигиены приложения."
                        
                    ready_html = html_content.replace("{ERROR_TITLE}", title_text)
                    ready_html = ready_html.replace("{ERROR_DETAILS}", details_text)
                    ready_html = ready_html.replace("{ERROR_TEXT}", danger_message)

                    base_url = QUrl.fromLocalFile(script_dir + "/")
                    self.browser.setHtml(ready_html, base_url)
                        
                    return False # Жестко прерываем функцию для опасного протокола

            # --- СЮДА код попадет, только если сайт БЕЗОПАСНЫЙ (http/https) ---
            clean_url = re.sub(r'[^a-zA-Z0-9\.\:\/\-\_\?\&\=\%]', '', user_url)
            
            # ЗАЩИТА: убрали stdout=PIPE, чтобы wget не зависал в памяти Windows при больших файлах
            cmd = ["wget", "--convert-links", "-P", "./load", clean_url]
            process = subprocess.Popen(cmd) 
            return process
            
        except Exception as e:
            print(f"[Error] Ошибка при запуске wget {e}")
            return False
    ...
    def add_new_tab(self, url=None):
        ...
        web_view.loadFinished.connect(lambda ok, wv=web_view: self.on_load_finished(wv, ok))
    ...
    def on_load_finished(self, web_view, ok):
        if ok:
            if web_view == self.get_current_web_view():
        ...
        failed_url = web_view.url().toString()
        # Исключаем зацикливание, если не удалось загрузить сам файл ошибки
        if failed_url.startswith("file://") or "Error.html" in failed_url or "about:blank" in failed_url:
            return

        print(f"[СЕТЬ] Ошибка загрузки: {failed_url}")
        
        # 1. Готовим тексты для маркеров {ERROR_TITLE} и {ERROR_DETAILS}
        title_text = "⚠️ Невозможно установить безопасное соединение"
        details_text = (
            f"Браузер FreeSearch заблокировал подключение к {failed_url}.<br><br>"
            "Этот сайт использует устаревший или поддельный сертификат безопасности (ошибка SSL). "
            "Злоумышленники могут пытаться похитить ваши пароли или данные банковских карт."
        )
        # 2. Формируем текст конкретно для висящего маркера {ERROR_TEXT}
        danger_message = f"Техническая информация: Сбой проверки цепочки сертификатов для хоста {urllib.parse.urlparse(failed_url).netloc}."

        # 3. Читаем твой Error.html напрямую, привязавшись к папке проекта через __file__
        script_dir = os.path.dirname(os.path.abspath(__file__))
        error_path = os.path.join(script_dir, "Error.html")
        ...
        if os.path.exists(error_path):
            ...
            # 4. Делаем ЧЕТКУЮ замену всех трех маркеров заглавными буквами, как в твоем HTML!
            ready_html = html_content.replace("{ERROR_TITLE}", title_text)
            ready_html = ready_html.replace("{ERROR_DETAILS}", details_text)
            ready_html = ready_html.replace("{ERROR_TEXT}", danger_message)

            # 5. Получаем Base URL, чтобы Chromium правильно подгрузил стили и градиенты
            base_url = QUrl.fromLocalFile(script_dir + "/")
            
            # Рендерим готовую страницу на вкладке
            web_view.setHtml(ready_html, base_url)
        else:
            # Фолбэк, если файла Error.html нет в папке
            fallback_html = f"<h1>{title_text}</h1><p>{details_text}</p><p>{danger_message}</p>"
            web_view.setHtml(fallback_html, web_view.url())
...
if __name__ == '__main__':
    ...
    def run_flask():
        try:
            # Укажи здесь точное имя переменной твоего Flask приложения (например flask_app)
    ...
    # Просто инициализируем главное окно
    window = BrowserWindow()
    window.show()
    
    # Передаем управление циклу через созданный в самом верху qt_app
