...
from PyQt6.QtCore import (
    ...
    QEasingCurve, QPoint, QRect, QEventLoop, QObject
)
...
class BrowserWindow(QMainWindow):
    def __init__(self):
        ...
        # Safe Browsing
        self.safe_browsing_api_key = ""  # ЗАМЕНИТЕ НА РЕАЛЬНЫЙ КЛЮЧ
    ...
    def on_load_finished(self, web_view, ok):
        ...
        if failed_url.startswith("file://") or "Error.html" in failed_url:
            return

        print(f"[СЕТЬ] Ошибка загрузки: {failed_url}")
        title = "⚠️ Невозможно установить безопасное соединение"
        details = (
            f"Браузер FreeSearch заблокировал подключение к <b>{failed_url}</b>.<br><br>"
        ...
        # Генерируем страницу ошибки из вашего Error.html
        error_html = SafeBrowsingChecker.error(title, details)
        web_view.setHtml(error_html, web_view.url())
